...
#def simulator(df,sentiment_df,n,k,s_sum):
    ...
    for day in tqdm(days , position = 0, leave = True):
        ...
        # top_df = mdf1.nsmallest(top_n, 'Buy_Score')
        # bottom_df = mdf2.nlargest(bottom_k, 'Sell_Score')
        # if top_df['Score'].max() == 0:
        #     top_df['Percent'] = 0
        #     doubt_list = []
        # else:
        #     pos_sums = sum(x for x in top_df['Score'] if x > 0)
        #     #score_sum = top_df['Buy_Score'].sum()
        #     top_df['Percent'] = [
        #         round(float(x/(pos_sums)),3) if x>0 and top_df['Score'].max()>0 
        #         else 0 for x in top_df['Score']]
...
#                 else 0 for x in top_df['Score']]
        ...
        if day == first_day:
            ...
            # prev_coefs = coefs
    print(current)
    if current/s_sum > 1.9061:
        return [n,k,d_coefs,current]
    else:
        return [None,None,None]
    ...
    if len(day_df['Daily Change'].dropna()) > 1:
        ...
        if len(day_df['Normalized_Volume'].dropna()) > 1:  
            ...
            if np.isnan(nVol)==True:
                nVol = 0
        if len(day_df['Sell_Ind'].dropna()) > 1:  
            column_4 = day_df['Sell_Ind']
            S_I = round(column_1.corr(column_4),3)
            if np.isnan(nVol)==True:
                S_I = 0       
    return [B_I,SA,nVol,S_I]
